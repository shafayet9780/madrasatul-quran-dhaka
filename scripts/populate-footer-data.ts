...
async function populateFooterData(): Promise<void> {
  try {
    console.log('🚀 Starting footer data population...');
    
    // Check if footer already exists
    const existingFooter = await client.fetch<{ _id: string } | null>(
    ...
    console.log('   • Quick Links:', footerData.quickLinks?.length || 0);
    console.log('   • Legal Links:', footerData.legalLinks?.length || 0);
